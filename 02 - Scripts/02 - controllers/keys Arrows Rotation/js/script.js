...
function startGame() {
    myGamePiece = new component(30, 30, "red", 225, 225); // Cria a peça do jogo com tamanho e posição especificados
...
var myGameArea = {
    ...
    stop : function() {
        clearInterval(this.interval); // Para a atualização da área do jogo
    },
    clear : function() {
...
function updateGameArea() {
    ...
    if (myGameArea.keys && myGameArea.keys[37]) {myGamePiece.moveAngle = -1; } // Tecla esquerda
    if (myGameArea.keys && myGameArea.keys[39]) {myGamePiece.moveAngle = 1; } // Tecla direita
    if (myGameArea.keys && myGameArea.keys[38]) {myGamePiece.speed= 1; } // Tecla cima
    if (myGameArea.keys && myGameArea.keys[40]) {myGamePiece.speed= -1; } // Tecla baixo
