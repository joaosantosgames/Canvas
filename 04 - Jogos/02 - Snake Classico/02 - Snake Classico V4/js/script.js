...
// Tipos de comida ruim com cores e penalidades diferentes
const badFoodTypes = [
    { color: 'black', points: -10, reduceSize: 1 },
    { color: 'brown', points: -20, reduceSize: 2 }
];

let food = spawnFood();
let badFood = spawnBadFood();
...
function moveSnake() {
    ...
    if (head.x === food.x && head.y === food.y) {
        ...
        score += badFood.points;
        for (let i = 0; i < badFood.reduceSize; i++) {
            snake.pop(); // Remove partes da cobra
        }
        badFood = spawnBadFood(); // Gera uma nova comida ruim
    } else {
...
// Função para desenhar a comida ruim
...
// Função para gerar comida ruim em uma posição aleatória
function spawnBadFood() {
    const foodX = Math.floor(Math.random() * canvas.width / gridSize) * gridSize;
    const foodY = Math.floor(Math.random() * canvas.height / gridSize) * gridSize;
    const randomBadFoodType = badFoodTypes[Math.floor(Math.random() * badFoodTypes.length)];
    return { x: foodX, y: foodY, ...randomBadFoodType };
}
